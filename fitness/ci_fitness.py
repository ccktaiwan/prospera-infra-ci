...
"""ci_fitness.py — 服務型結構 fitness(SRAF L5,真實檢查非 stub)。
...
def check():
    r={
        ...
        "check_api_contracts": os.path.isfile(os.path.join(_R,"check_api_contracts.py")),
        "check_architecture_dependencies": os.path.isfile(os.path.join(_R,"check_architecture_dependencies.py")),
        "check_repository_registry": os.path.isfile(os.path.join(_R,"check_repository_registry.py")),
    }
...
def main():
    ...
    print(f"[ci-fitness] {r['passed']}/{r['total']} pass")
